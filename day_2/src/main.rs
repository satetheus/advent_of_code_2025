...
fn main() {
    ...
    for range in &ranges {
        let invalids = find_invalid_ids(range);
        total_p1 += invalids.0.iter().sum::<i64>();
        total_p2 += invalids.1.iter().sum::<i64>();
    }
...
fn process_file(filename: impl AsRef<Path>) -> Vec<Vec<i64>> {
    ...
    let split_lines: Vec<Vec<i64>> = lines.iter().map(|l| split_str!(l, '-')).collect();
...
mod tests {
    ...
    fn test_find_invalid_ids() {
        assert_eq!(
            find_invalid_ids(&[11, 22]),
            (vec![11, 22], HashSet::from([11, 22]))
        );
        assert_eq!(
            find_invalid_ids(&[95, 115]),
            (vec![99], HashSet::from([99, 111]))
        );
        assert_eq!(
            find_invalid_ids(&[998, 1012]),
            (vec![1010], HashSet::from([999, 1010]))
        );
        assert_eq!(
            find_invalid_ids(&[1188511880, 1188511890]),
            (vec![1188511885], HashSet::from([1188511885]))
        );
        assert_eq!(
            find_invalid_ids(&[222220, 222224]),
            (vec![222222], HashSet::from([222222]))
        );
        assert_eq!(
            find_invalid_ids(&[1698522, 1698528]),
            (vec![], HashSet::from([]))
        );
        assert_eq!(
            find_invalid_ids(&[446443, 446449]),
            (vec![446446], HashSet::from([446446]))
        );
        assert_eq!(
            find_invalid_ids(&[38593856, 38593862]),
            (vec![38593859], HashSet::from([38593859]))
        );
        assert_eq!(
            find_invalid_ids(&[2121212118, 2121212124]),
            (vec![], HashSet::from([2121212121]))
        );
    }
